...
import { CollectionTierRemainingSupply } from "flair-sdk";
...
export default function Play() {
  ...
  const { contract: characterContract } = useContract(CHARACTER_EDITION_ADDRESS, "edition-drop" );
  const { contract: pickaxeContract } = useContract(PICKAXE_EDITION_ADDRESS,    "edition-drop"  );
  const { contract: tokenContract } = useContract(GOLD_GEMS_ADDRESS, "token");

  if (!address) {
    return (
      <div className={styles.container}>
        <ConnectWallet colorMode="light" accentColor="#a955f794" />
      </div>
    );
  }
  ...
  return (
    <div className={styles.container}>
      ...
      {pickaxeContract && miningContract ? (
        <>
          <h2 style={{color: "#ff00fc"}} className={`${styles.noGapTop} ${styles.noGapBottom}`}>
            Your Owned Energy
          </h2>
          <div
            style={{
      ...
      {pickaxeContract && tokenContract ? (
        <>
          <h2 style={{color: "#ff00fc"}} className={`${styles.noGapTop} ${styles.noGapBottom}`}>Shop</h2>
          <div
            style={{
              width: "100%",
              minHeight: "10rem",
